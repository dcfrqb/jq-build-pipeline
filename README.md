...
│   └── coverage/           # Coverage артефакты и отчёты
└── reports/                # Отчёты о сборке
    └── build_report_*.txt  # Отчёты с информацией о сборке
```
...
- Отчёт о покрытии: `artifacts/coverage/coverage_<ревизия>.txt`
- Файл lcov: `artifacts/coverage/coverage_<ревизия>.info`

## Отчёты

После каждой сборки создаётся отчёт в директории `reports/`:
